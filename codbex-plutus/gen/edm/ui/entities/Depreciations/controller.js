angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'codbex-plutus-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "entities" && e.view === "Depreciations" && (e.type === "page" || e.type === undefined));
			$scope.entityActions = response.filter(e => e.perspective === "entities" && e.view === "Depreciations" && e.type === "entity");
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("Depreciations", `Unable to count Depreciations: '${response.message}'`);
					return;
				}
				if (response.data) {
				...
				request.then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("Depreciations", `Unable to list/filter Depreciations: '${response.message}'`);
						return;
					}
		...
		$scope.openDetails = function (entity) {
			$scope.selectedEntity = entity;
			messageHub.showDialogWindow("Depreciations-details", {
				action: "select",
				entity: entity,
		...
		$scope.openFilter = function (entity) {
			messageHub.showDialogWindow("Depreciations-filter", {
		...
		$scope.deleteEntity = function (entity) {
			...
			messageHub.showDialogAsync(
				...
				`Are you sure you want to delete Depreciations? This action cannot be undone.`,
				[{
					id: "delete-btn-yes",
					type: "emphasized",
					label: "Yes",
				...
				if (msg.data === "delete-btn-yes") {
					entityApi.delete(id).then(function (response) {
						if (response.status != 204) {
							messageHub.showAlertError("Depreciations", `Unable to delete Depreciations: '${response.message}'`);
							return;
						}
						$scope.loadPage($scope.dataPage, $scope.filter);
