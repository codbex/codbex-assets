...
export class DepreciationRepository {
    ...
    public customDataCount(): number {
        const resultSet = query.execute('SELECT COUNT(*) AS COUNT FROM "DEPRECIATION"');
    ...
    private async triggerEvent(data: DepreciationEntityEvent | DepreciationUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-plutus-entities-Depreciation", ["trigger"]);
        ...
        producer.topic("codbex-plutus-entities-Depreciation").send(JSON.stringify(data));
    }
}
