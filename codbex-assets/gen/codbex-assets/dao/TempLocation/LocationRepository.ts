...
export class LocationRepository {
    ...
    public customDataCount(): number {
        const resultSet = query.execute('SELECT COUNT(*) AS COUNT FROM "CODBEX_PLUTUS_LOCATION"');
    ...
    private async triggerEvent(data: LocationEntityEvent | LocationUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-assets-TempLocation-Location", ["trigger"]);
        ...
        producer.topic("codbex-assets-TempLocation-Location").send(JSON.stringify(data));
    }
}
